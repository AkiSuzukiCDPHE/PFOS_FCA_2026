...
PFOSData_4 <- PFOSData_3 %>% mutate(MDL=as.character(MDL)) |> 
  mutate(Result=case_when(Qualifier %in% c("U", "BDL") ~ MDL,
                          TRUE~ (Result)))
...
PFOSData_Merged2 <- subset(PFOSData_Merged1, !Species %in% c("Crayfish", "Trout - unspecified")) |> mutate(Sample_Year=year(Sample_Date))
...
library("writexl")
write_xlsx(PFOSData_Clean, ("03_Clean_Data/PFOS_CleanedMaster_2026.xlsx"))
