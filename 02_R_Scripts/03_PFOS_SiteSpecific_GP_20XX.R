...
PFOS_Clean_Original <- read_excel("X:/Shared drives/_CDPHE TEEO TARA/PFAS 🔥/Data Integration and Assessment/Fish/FCAs/PFOS FCAs/Annual FCA Updates/PFOS_FCA_2024/03_Clean_Data/PFOS_Clean_2020_2023_VALIDATED.xlsx")
...
PFOS_SS2=PFOS_SS %>%
  mutate(GP_MealsPerMonth=case_when(Average_Result>0 & Average_Result <=.4 ~24,
...
# NO in 2024 because it was the first year of site-specific advisories.
...
PFOS_SS3 <- left_join(PFOS_SS2, Hg_Statewide_Final_2024 %>% select(Species_Codes, GP_MealsPerMonth, Commonly_Consumed),
                      by = "Species_Codes")
...
PFOS_SS5a <- PFOS_SS4 %>% filter(GP_MealsPerMonthSW <=8) # Species that have a statewide advisory

PFOS_SS5b <- PFOS_SS4 %>% filter(GP_MealsPerMonthSW > 8) # Species that DO NOT have a statewide advisory
...
PFOS_GP_SS_Merged <- bind_rows(PFOS_SS6a, PFOS_SS6b)
